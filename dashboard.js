d3.csv("all-buoy-data.csv").then(d => chart(d))
...
function chart(csv) {
	...
	svg.append("text")
		...
		.text( d=> 'Water Temperature At Depth');
	...
	.transition().duration(0)
		...
		//.attr("fill", d => { return d.valid ? colorRange(d.value) : 'gray';})
		.attr("fill", d => {
			//var rgbStr = colorRange(d.prevVal) + colorRange(d.value);
			//rgbStr = rgbStr.replace(/[^a-zA-Z0-9]/g, "");
	...
	function setupWindChart(csv) {
		...
		.text( d=> 'Wind Speed and Direction');
